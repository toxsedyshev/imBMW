...
namespace imBMW.iBus.Devices.Real
{
    public static class Navigation
    {
        ...
        {
            var speed = InstrumentClusterElectronics.Speed;
    ...
    public class Coordinate
    {
        ...
        public Coordinate(float latitude, float longitude, float altitude,
                          short speed, DateTime time, bool isFixed)
        {
            Altitude = altitude;
            Latitude = latitude;
            Longitude = longitude;
        ...
        private static double Distance(float lat1, float lng1, float lat2, float lng2, float radius)
        {
            ...
            var lat = Tools.Math.ToRadians(lat2 - lat1);
			var lng = Tools.Math.ToRadians(lng2 - lng1);
			var sinLat = Tools.Math.Sin(0.5 * lat);
			var sinLng = Tools.Math.Sin(0.5 * lng);
			var cosLat1 = Tools.Math.Cos(Tools.Math.ToRadians(lat1));
			var cosLat2 = Tools.Math.Cos(Tools.Math.ToRadians(lat2));
            var h1 = sinLat * sinLat + cosLat1 * cosLat2 * sinLng * sinLng;
			var h2 = Tools.Math.Sqrt(h1);
			var h3 = 2 * Tools.Math.Asin(Tools.Math.Min(1, h2));
            return radius * h3;
        }
