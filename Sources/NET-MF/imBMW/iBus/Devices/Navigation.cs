...
namespace imBMW.iBus.Devices.Real
{
    ...
    public class Coordinate
    {
        ...
        private static double Distance(float lat1, float lng1, float lat2, float lng2, float radius)
        {
            ...
            var lat = MathEx.ToRadians(lat2 - lat1);
            var lng = MathEx.ToRadians(lng2 - lng1);
            var sinLat = System.Math.Sin(0.5 * lat);
            var sinLng = System.Math.Sin(0.5 * lng);
            var cosLat1 = System.Math.Cos(MathEx.ToRadians(lat1));
            var cosLat2 = System.Math.Cos(MathEx.ToRadians(lat2));
